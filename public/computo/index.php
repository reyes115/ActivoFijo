...
if ( $_SERVER[ 'REQUEST_METHOD' ] === 'POST' ) {
    ...
    if ( isset( $_POST[ 'eliminar' ] ) ) {
        ...
        delete_computo( $conexion, $id );
    }
}

$encabezados = array( "ID", "Tipo", "Marca", "Modelo", "No.serie", "Procesador", "Memoria RAM", "Almacenamiento", "Estado", "Color", "Cargador", "Usuario Asignado", "Ver más" );

$datos_compu = view_compu( $conexion );
...
<div class="layoutSidenav_content">
  <div class="container-fluid"> 
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800" align="center"><strong>EQUIPOS DE COMPUTO</strong></h1>
    <!--Botones-->
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        ...
        if ( $access[ 'computo' ] == 1 ) {
            ?>
        <a href="new_computo" class="btn bg-secundario btn-icon-split btn-sm"> <span class="icon text-white-50"> <i class="fas fa-plus"></i> </span> <span class="text text-white">Nuevo Registro</span> </a>
        <?php
        }
        ?>
      ...
      <div class="card-body">
        <div class="table-responsive" >
          <table class=" table table-bordered display table-hover" id="dataTable" width="100%" cellspacing="0" style="font-size: 12px;">
            ...
            <tbody>
              <?php
              while ( $row = mysqli_fetch_array( $datos_compu ) ) {
                  switch ( $row[ 'estado' ] ) {
                      case 1:
                          $estado = "Nuevo";
                          break;
                  ...
                  $codigoQR = $row[ 'QRKey' ];
                  $id_registro = $row[ 'id_compu' ];
                  $name_delete = $row[ 'codigo' ];
                  ?>
              <tr ondblclick="abrirEnlace('ver_equipo<?php echo $codigoQR ?>')">
                ...
                ?></td>
                <td align="center"><acronym title="Ver más "> <a href="ver_equipo<?php echo $codigoQR ?>" class="btn btn-success  btn-sm" > <i class="fas fa fa-share"></i></a></acronym></td>
                <?php
...
<script src="TablaComputoJs"></script> 
