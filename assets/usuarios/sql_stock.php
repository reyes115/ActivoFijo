...
function insert_stock( $conexion, $propietario, $tipo, $caracteristicas, $estado, $cantidad, $almacenamiento, $usuarioAsignado ) {
    ...
    $stmt = $conexion->prepare( "SELECT MAX(cons) as cons FROM stock LIMIT 1" );
    $stmt->execute();
    ...
    // guardar archivos
    $directorio = $_SERVER[ 'DOCUMENT_ROOT' ] . "/uploads/mobiliario/$codigo"; //Declaramos un  variable con la ruta donde guardaremos los archivos
    ...
    $t_qr = "stock";
    $urlQr = "https://ceers.innovet.com.mx/ver_mobiliario" . $QRKey;
    ...
    mysqli_stmt_bind_param( $stmt, "ssssssssss", $codigo, $tipo, $caracteristicas, $estado, $cantidad, $almacenamiento, $propietario, $usuarioAsignado, $QRKey, $cons );

    $usu_usuario = $_SESSION[ 'user_name' ];
    $accion = "Registro nuevo mobiliario: " . $codigo;
...
function view_mob_asig( $conexion, $id ) {
    $stmt = mysqli_prepare( $conexion, "SELECT *,DATE_FORMAT(fecha_relevo, '%d-%m-%Y') AS fecha_formateada FROM `previous_devices` WHERE `id_devices`= ? AND `type`= 'mobiliario';" );
...
function delete_stock($conexion, $id) {
    //variables para el historial
    $usu_usuario = $_SESSION['user_name'];
    $accion = "Elimino el registro del mobiliario: " . $id;

    $stmt = mysqli_prepare($conexion, "UPDATE `stock` SET `activo`='0' WHERE `id_stock` = ?");
